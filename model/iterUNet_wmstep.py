...
from .time_embedding import (
    ...
    resize_and_project_time_embedding,
)
...
class iterUNet(nn.Module):
    def __init__(
        ...
        self.time_embeddings = [
            get_sinusoidal_positional_encoding(
                torch.tensor([t]), self.time_embedding_dim
            )
    ...
    def forward(self, x: torch.Tensor):
        if self.training:
            ...
            for i in range(self.num_iterations):
                ...
                hidden_state, cell_state = self.conv_lstm(
                    d0, (hidden_state, cell_state) if hidden_state is not None else None
                )
                d0 = hidden_state

                d0_emd = resize_and_project_time_embedding(
                    self.mtp0(time_emb), d0.shape[2:], self.time_projection0
                )
                d0 = d0 + d0_emd
                d1 = self.down_1(d0)
                d1_emd = resize_and_project_time_embedding(
                    self.mtp1(time_emb), d1.shape[2:], self.time_projection1
                )
                d1 = d1 + d1_emd
                d2 = self.down_2(d1)
                d2_emd = resize_and_project_time_embedding(
                    self.mtp2(time_emb), d2.shape[2:], self.time_projection2
                )
                d2 = d2 + d2_emd
                d3 = self.down_3(d2)
                d3_emd = resize_and_project_time_embedding(
                    self.mtp3(time_emb), d3.shape[2:], self.time_projection3
                )
                d3 = d3 + d3_emd
                d4 = self.down_4(d3)
                d4_emd = resize_and_project_time_embedding(
                    self.mtp4(time_emb), d4.shape[2:], self.time_projection4
                )
                d4 = d4 + d4_emd

                u4 = self.upcat_4(d4, d3)
                u4_emd = resize_and_project_time_embedding(
                    self.mtp3(time_emb), u4.shape[2:], self.time_projection3
                )
                u4 = u4 + u4_emd
                u3 = self.upcat_3(u4, d2)
                u3_emd = resize_and_project_time_embedding(
                    self.mtp2(time_emb), u3.shape[2:], self.time_projection2
                )
                u3 = u3 + u3_emd
                u2 = self.upcat_2(u3, d1)
                u2_emd = resize_and_project_time_embedding(
                    self.mtp1(time_emb), u2.shape[2:], self.time_projection1
                )
                u2 = u2 + u2_emd
                u1 = self.upcat_1(u2, d0)
                u1_emd = resize_and_project_time_embedding(
                    self.mtp0(time_emb), u1.shape[2:], self.time_projection0
                )
                u1 = u1 + u1_emd

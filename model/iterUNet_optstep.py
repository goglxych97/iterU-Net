    ...
    def forward(self, x: torch.Tensor):
        if self.training:
            ...
            for j in range(self.num_iterations-1):
                if j==0:
                    x_post = iter_outputs[0]
                else:
                    x_post = syn_outputs[-1]
                    
                time_emb = self.time_embeddings[j+1].to(x.device) #j=0->(pre,2st)
                d0 = self.conv_0(torch.cat((x_pre, x_post,), axis=1))
                hidden_state, cell_state = self.conv_lstm(d0, (hidden_state, cell_state) if hidden_state is not None else None)
                d0 = hidden_state
                ...
                syn_output = self.final_conv(u1) + x_post
                syn_outputs.append(syn_output)                
                
            return tuple(iter_outputs+syn_outputs)
            ...
            for j in range(self.num_iterations):
                if i==0:
                    x_post = x[:, 1, :, :, :].unsqueeze(1)
                else:
                    x_post = syn_outputs[-1]
                
                time_emb = self.time_embeddings[j].to(x.device)
                d0 = self.conv_0(torch.cat((x_pre, x_post,), axis=1))
                hidden_state, cell_state = self.conv_lstm(d0, (hidden_state, cell_state) if hidden_state is not None else None)
                d0 = hidden_state
